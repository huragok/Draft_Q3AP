...
high-quality solutions over the QAPLIB dataset. In this
...
design with a LDPC coded system based on~\cite{hochwald2003achieving}, where we
use a LDPC code with length $L=2400$, coding rate of $0.75$ and a Monte-Carlo
run of up to 2000 LDPC frames. Despite that our Q3AP MoDiv scheme depends on
noise power and statistical CSI, in this simulation we deliberately uses Q3AP
MoDiv schemes designed for $E_b/N_0=-2dB$, AWGN channel and test it on Rician
channel where $K=10$, $a=1$ and varying $E_b/N_0$ to demonstrate its robustness.
The coded BER results for 16-QAM and 32-QAM are plotted in
Fig.~\ref{fig:coded_BER16QAM} and Fig.~\ref{fig:coded_BER32QAM}, respectively.
We can see that the Q3AP MoDiv solution provides additional gain over the
heuristic MoDiv and no MoDiv scheme even under imperfect design parameters. Also
we notice that the Q3AP MoDiv provides larger performance gain for 32-QAM than
