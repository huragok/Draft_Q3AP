...
symbols. This approach is particularly effective when retransmitting
packets in hybrid-ARQ (HARQ) systems. In this paper, we study the optimization
of MoDiv in a cooperative relay-HARQ network where the source and relay jointly
share the task of retransmission through coordination to reduce the bit error
rate (BER).
    % (how they do)
    % (how we do)
We formulate the minimum-BER MoDiv design optimization problem rigorously into a
quadratic three-dimensional assignment problem (Q3AP), which is solved with an
efficient modified Iterated local search method. Numerical results demonstrate
the performance gain of the proposed MoDiv scheme, and show that when the
channels are heavily fading, a comparable BER reduction can be achieved with a
heuristic MoDiv desgin.
    % (main results)
\end{abstract}
...
is~\cite{schwartz1995communication}
\begin{align}
