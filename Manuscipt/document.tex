...
\begin{abstract}
    ...
    efficient modified Iteratied Local Search (ILS) method to find the optimal
    CoRe solution.
    % (main results)
...
benlic2015memetic} provides very high-quality solutions over the cases in QAPLIB~\cite{burkard1997qaplib}.
...
\begin{itemize}
    ...
    the destination coordinatedly, practically forming a 2-by-1 multiple-input single-   
    output MISO system.
    \item In our cooperative relay-HARQ settings, we formulate the CoRe design
    into a Q3AP problem. Since Q3AP/QAP formulation is NP-complete, we adopt an efficient
...
source convert a bit sequence of length $\log_2Q$ into symbols with
Gray mapping $\psi_0:
...
receiving a request for retransmission, the second phase is started. This time the source and the relay
...
that 16-QAM does not have enough symmetry to exploit as 16-PSK
...
a 2-opt neighborhood search. A first-improvement rule is used, which means when
ever an reduction in objective function is made the mapping is updated. The
perturbation in S3 is performed by exchanging the mapping of $k_p$ indices,
where $k_p\in [k_{p,min}, k_{p,max}]$ is adaptively increased if the
purterbation produces a no better solution or reset to $k_{p,min}$ otherwise.
